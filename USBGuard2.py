...
# Функция для получения списка логических дисков
def get_drives():
    drives = win32api.GetLogicalDriveStrings()
    drives = drives.split('\000')[:-1]
    return set(drives)

# Функция для мониторинга USB-устройств
def monitor_usb():
    ...
    while True:
        ...
        if drives_before != drives_after:
            ...
            bot.send_message(CHAT_ID, "USB device event detected")
            photo = take_photo()
...
def detect_motion():
    ...
    while motion_detection_active:
        ...
        thresh = cv2.threshold(delta_frame, 50, 255, cv2.THRESH_BINARY)[1]  # Увеличили порог до 50
        ...
        if motion_detected and not recording_video:
            ...
            # Проверка флага после записи видео
            if motion_detection_active:
                continue

        time.sleep(0.5)  # Временной интервал между проверками кадров
...
# Запуск мониторинга USB устройств в отдельном потоке
